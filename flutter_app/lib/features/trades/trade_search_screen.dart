...
class TradeMatchResult {
  ...
  factory TradeMatchResult.fromJson(Map<String, dynamic> j) {
    ...
    return TradeMatchResult(
      ...
      routePattern:       j['routePattern'] ?? '',
      legTypePattern:     List<String>.from(j['legTypePattern'] ?? []),
      blockHours:         (j['blockHours'] ?? 0).toDouble(),
      dutyHours:          (j['dutyHours'] ?? 0).toDouble(),
      restAfterHours:     (j['restAfterHours'] ?? 0).toDouble(),
      income:             (j['income'] ?? 0).toDouble(),
      legalityPassed:     leg['passed'] ?? false,
      hasCarryOver:       j['hasCarryOver'] ?? false,
      hasDeadhead:        j['hasDeadhead'] ?? false,
      legalitySummary:    leg['summary'] ?? '',
      ...
      fatigueScore:       (fat['score'] ?? 0).toDouble(),
      fatigueFactors:     List<String>.from(fat['factors'] ?? []),
      fatigueComparison:  [fat['comparison']?.toString() ?? ''],
      tradeScore:         (sc['total'] ?? 0).toDouble(),
      scoreReasons:       List<String>.from(sc['reasons'] ?? []),
